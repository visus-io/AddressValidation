...
- **`#pragma` indentation is a known ReSharper/`dotnet format` disagreement.** `.editorconfig` sets `resharper_csharp_indent_preprocessor_other = usual_indent`, so ReSharper indents `#pragma` lines (e.g. `#pragma warning disable`) to match the surrounding code block. `dotnet format` (Roslyn's formatter) has no `csharp_*` key for preprocessor-directive indentation and always flattens `#pragma` to column 0, regardless of the `resharper_*` setting. When a `#pragma` line is indented to match its block, leave it indented — that is the intended, ReSharper-driven style, not a defect — even though `dotnet format whitespace --verify-no-changes` will flag it. This is a pre-existing tooling gap, not something to "fix" by reformatting.
