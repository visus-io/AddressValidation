    ...
    where T : class
{
    ...
    public ValueTask<IReadOnlyList<IValidationResult>> ExecuteAsync(T instance, IReadOnlyList<int> requestIndexes, CancellationToken cancellationToken = default)
    {
        ...
        ArgumentNullException.ThrowIfNull(requestIndexes);

        return ExecuteInternalAsync(instance, requestIndexes, cancellationToken);
    }
    ...
    private async ValueTask<IReadOnlyList<IValidationResult>> ExecuteInternalAsync(T instance, IReadOnlyList<int> requestIndexes, CancellationToken cancellationToken)
    {
        List<ISet<ValidationState>> perItemResults = new(requestIndexes.Count);
        for ( int i = 0; i < requestIndexes.Count; i++ )
        {
            perItemResults.Add(new HashSet<ValidationState>());
        }

        if ( !await PreValidateAsync(instance, requestIndexes, perItemResults, cancellationToken).ConfigureAwait(false) )
        {
            return BuildResults(perItemResults);
        }

        await ValidateAsync(instance, requestIndexes, perItemResults, cancellationToken).ConfigureAwait(false);
