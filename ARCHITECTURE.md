...
flowchart TD
    ...
    MapResp["IBatchApiResponseMapper\nGetSharedCustomResponseData(response) computed once,\nthen for each item: HasErrors → EmptyAddressValidationResponse,\notherwise Map(...) at its sent-batch position"]
...
- **Response-level custom data is computed once per batch, not once per item.** `IBatchApiResponseMapper<T>.GetSharedCustomResponseData(response)` is called exactly once per `ValidateManyAsync` call, before the per-item mapping loop, and the result is passed into every `Map(...)` call as `sharedCustomResponseData`. This exists because `response` is the same `TApiResponse` instance for every item in the batch, so any response-level data it exposes (e.g. FedEx's `customerTransactionId`) is invariant across the whole call — recomputing it per item would repeat identical work up to `MaxBatchSize` times for no benefit.
