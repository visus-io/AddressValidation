...
`ApiMessageFormatter` is a shared helper that turns a provider's error or alert `(code, message)` pair into a `ValidationState`. Every provider API returns errors and alerts in this same shape, so each response validator calls `ApiMessageFormatter.CreateError(code, message)` or `CreateWarning(code, message)` instead of re-implementing the same formatting logic. The helper falls back to the code alone when the message is empty, and to the message alone when the code is empty, since different providers omit one or the other field.
