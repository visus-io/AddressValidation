...
`ApiMessageFormatter` is a shared helper that turns a provider's error or alert `(code, message)` pair into a `ValidationState`. Every provider API returns errors and alerts in this same shape. Each response validator calls `ApiMessageFormatter.CreateError(code, message)` or `CreateWarning(code, message)` instead of re-implementing the same formatting logic. Different providers omit the code or the message field. The helper falls back to the code alone when the message is null, empty, or white space. It falls back to the message alone when the code is null, empty, or white space. It throws `ArgumentException` when both are null, empty, or white space.
